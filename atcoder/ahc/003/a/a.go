...
func NewGraph(starts, ends []int, costs []float64) Graph {
	E := len(starts)
	maxV := slices.Max(starts)
	endMax := slices.Max(ends)
	if maxV < endMax {
		maxV = endMax
	}
	maxV += 1
	...
	adjacencyList := make([][]int, maxV)
	for i := 0; i < E; i++ {
		adjacencyList[starts[i]] = append(adjacencyList[starts[i]], i)
		adjacencyList[ends[i]] = append(adjacencyList[ends[i]], i)
	}
	return Graph{
		V:             maxV,
		E:             E,
...
func makeGridGraph(n int) Graph {
	var starts, ends []int
	var costs []float64
	for i := 0; i < n; i++ {
		for j := 0; j < n-1; j++ {
	...
	return NewGraph(starts, ends, costs)
}
...
func NewEstimator(mu []float64, sigma [][]float64) *Estimator {
	return &Estimator{
		n:             len(mu),
		mu:            mu,
		sigma:         sigma,
...
func (e *Estimator) observe(indices []int, observedY float64, noiseVar float64) {
	var sxy []float64
	for i := 0; i < e.n; i++ {
	...
	for i := 0; i < e.n; i++ {
		e.mu[i] += sxy[i] * muDiff / syy
	}
	for _, i := range indices {
		for _, j := range indices {
			e.sigma[i][j] -= sxy[i] * sxy[j] / syy
		}
	}
}

func main() {
	graph := makeGridGraph(30)
	for i := 0; i < 1000; i++ {
		sx, sy, tx, ty := readInt(), readInt(), readInt(), readInt()
		var path string
		diffx := tx - sx
		if diffx >= 0 {
			path += strings.Repeat("D", diffx)
		} else {
			path += strings.Repeat("U", -diffx)
		}
		diffy := ty - sy
		if diffy >= 0 {
			path += strings.Repeat("R", diffy)
		} else {
			path += strings.Repeat("L", -diffy)
		}
		fmt.Println(path)
		_ = readInt()
	}
