...
// 右開区間
func isOverlapped(al, ar, bl, br int) bool {
	return al < br && bl < ar
}
...
func main() {
	b, c := readInt(), readInt()
	pl, pr := b-c/2, b+(c-2)/2
	ml, mr := -b-(c-1)/2, -b+(c-1)/2
	allSum := pr - pl + 1 + mr - ml + 1
	if !isOverlapped(pl, pr+1, ml, mr+1) {
		fmt.Println(allSum)
		return
	}
	overlapPart := min(mr, pr) - max(pl, ml) + 1
	fmt.Println(allSum - overlapPart)
}
