...
func main() {
	...
	for k := 0; k < n; k++ {
		for s := 0; s < n; s++ {
			for t := 0; t < n; t++ {
				c := min(edges[k][s][t], edges[k][s][k]+edges[k][k][t])
				if c != inf {
					sum += c
				}
				if k < n-1 {
...
func dijkstra(n int, edges []map[int]int, start int, k int) []int {
