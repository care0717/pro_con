...
func main() {
	...
	left := int64(minBaseNum)
	right := int64(math.Pow(float64(m), 1/float64(len(x)-1))) + 1
	for left+1 < right {
		mid := (left + right) / 2
		c := calcBaseNum(x, mid, m)
		if c <= m {
			left = mid
		} else {
			right = mid
	...
	leftNum := calcBaseNum(x, left, m)
	rightNum := calcBaseNum(x, right, m)
	if leftNum <= m && m < rightNum {
		fmt.Println(left - int64(minBaseNum) + 1)
	} else if m < leftNum && m < rightNum {
		fmt.Println(0)
	} else {
		fmt.Println(leftNum, m, rightNum)
	}

}

func calcBaseNum(x string, base int64, limit int64) int64 {
	n := len(x)
	var res int64
	for i := 0; i < n; i++ {
		num, _ := strconv.Atoi(string(x[n-1-i]))
		if i == 0 {
			res += int64(num)
		} else if i == 1 {
			res += int64(num) * base
		} else {
			if float64(limit)/float64(num) < math.Pow(float64(base), float64(i)) || limit < res {
				return limit + 1
			}
			res += int64(float64(num) * math.Pow(float64(base), float64(i)))
		}
	}
	return res
}
