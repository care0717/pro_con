...
func main() {
	...
	for a := 0; a < length; a++ {
		e[a] = make([][]float64, length)
		for b := 0; b < length; b++ {
			e[a][b] = make([]float64, length)
		}
	}
	for a := 99; a >= 0; a-- {
		for b := 99; b >= 0; b-- {
			for c := 99; c >= 0; c-- {
				e[a][b][c] = float64(a)/float64(a+b+c)*(1.0+e[a+1][b][c]) + float64(b)/float64(a+b+c)*(1.0+e[a][b+1][c]) + float64(c)/float64(a+b+c)*(1.0+e[a][b][c+1])
			}
		}
	}
	fmt.Println(e[A][B][C])
