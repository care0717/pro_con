...
func main() {
	n, m := readInt(), readInt()
	tmpEdges := make([]map[int][]int, n)
	edges := make([]map[int]int, n)
	for i := 0; i < n; i++ {
		tmpEdges[i] = make(map[int][]int)
		edges[i] = make(map[int]int)
	}
	for i := 0; i < m; i++ {
		a, b, c := readInt()-1, readInt()-1, readInt()
		tmpEdges[a][b] = append(tmpEdges[a][b], c)
	}
	for k, v := range tmpEdges {
		for node, costs := range v {
			edges[k][node] = min(costs...)
		}
	}
	for i := 0; i < n; i++ {
		fmt.Println(dijkstra(n, edges, i))
	}
}
