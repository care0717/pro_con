...
func main() {
	x, y, r := readFloat64(), readFloat64(), readFloat64()
	intx, inty, intr := int64(x*10000), int64(y*10000), int64(r*10000)
	ymin, ymax := inty-intr, inty+intr
	var count int64
	i := int64(math.Ceil(float64(ymin)/10000)) * 10000

	for i <= ymax {
		count += solve2(intr, inty, intx, i)
		i += 10000
	}
...
func solve(intr, inty, intx, i int64) int64 {
	tmp := math.Sqrt(float64(intr*intr - (inty-i)*(inty-i)))

	includeXmin := int64(math.Ceil((float64(intx) - tmp) / 10000))
	includeXmax := int64(math.Floor((float64(intx) + tmp) / 10000))
	return includeXmax - includeXmin + 1
}

func solve2(intr, inty, intx, i int64) int64 {
	chord2 := intr*intr - (inty-i)*(inty-i)
	chord := int64(math.Sqrt(float64(chord2)))
	if chord*chord > chord2 {
		chord--
	}
	includeXmin := (intx - chord) / 10000
	if (intx-chord)%10000 != 0 && intx-chord > 0 {
		includeXmin++
	}
	includeXmax := (intx + chord) / 10000
	if (intx+chord)%10000 != 0 && intx+chord < 0 {
		includeXmax--
	}
	return includeXmax - includeXmin + 1
