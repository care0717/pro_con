...
func main() {
	...
	var winPair [][]int
	for i := 1; i <= 9; i++ {
		takahashiScore := calcScore(fmt.Sprintf("%s%d", s, i))
		for j := 1; j <= 9; j++ {
			aokiScore := calcScore(fmt.Sprintf("%s%d", t, j))
			if takahashiScore > aokiScore {
				winPair = append(winPair, []int{i, j})
			}
		}
	}
	cards := make(map[int]int)
	for i := 1; i <= 9; i++ {
		cards[i] = k - strings.Count(s, fmt.Sprintf("%d", i)) - strings.Count(t, fmt.Sprintf("%d", i))
	}
	var prob float64
	for _, p := range winPair {
		prob += calcProb(cards, 9*k-8, p)
	}
	fmt.Println(prob)
...
func calcScore(s string) int {
	var sum int
	for i := 1; i <= 9; i++ {
		c := strings.Count(s, fmt.Sprintf("%d", i))
		sum += i * int(math.Pow(10, float64(c)))
	}
	return sum
}

func calcProb(cards map[int]int, cardNum int, pair []int) float64 {
	p1 := float64(cards[pair[0]]) / float64(cardNum)
	var p2 float64
	if pair[0] == pair[1] {
		p2 = float64(cards[pair[1]]-1) / float64(cardNum-1)
	} else {
		p2 = float64(cards[pair[1]]) / float64(cardNum-1)
	}
	return p1 * p2
}
