...
func main() {
	...
	for i := 0; i < m; i++ {
		l := readInt() - 1
		if _, ok := sections[l]; !ok {
			sections[l] = make([]int, n)
		}
		r := readInt() - 1
		sections[l][r]++
	}
	cumSections := make(map[int]CumulativeSum)
	for l, list := range sections {
		cumSections[l] = NewCumulativeSum(list)
	}
	for i := 0; i < q; i++ {
		l, r := readInt()-1, readInt()-1
		var sum int
		for j := l; j <= r; j++ {
			if cum, ok := cumSections[j]; ok {
				sum += cum.Get(0, r+1)
			}
		}
		fmt.Println(sum)
