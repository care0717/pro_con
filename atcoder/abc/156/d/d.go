...
func main() {
	...
	all := modPow(2, n, mod) - 1%mod
	countA := countPattern(n, a, mod)
	countB := countPattern(n, b, mod)

	result := all - countA - countB%mod
	for result < 0 {
		result += mod
	}
	fmt.Println(result)
...
func countPattern(n, a, mod int) int {
	x := 1
	y := 1
	for i := 0; i < a; i++ {
		x = x * (n - i) % mod
	...
	inverseY := modPow(y, mod-2, mod)
	return x * inverseY % mod
}
