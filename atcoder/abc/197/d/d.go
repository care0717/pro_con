...
func main() {
	...
	xn2, yn2 := readFloat64(), readFloat64()
	diagonalLength := math.Sqrt((xn2-x0)*(xn2-x0) + (yn2-y0)*(yn2-y0))
	sideLen := diagonalLength * math.Sin(math.Pi/float64(n))
	basePoint := Point{
		X: sideLen * math.Cos(math.Pi/float64(n)),
		Y: sideLen * math.Sin(math.Pi/float64(n)),
	}
	rotatePoint := Point{
		X: xn2 - x0,
		Y: yn2 - y0,
	...
	} else if rotatePoint.X > 0 {
		theta := math.Atan(rotatePoint.Y/rotatePoint.X) - math.Pi/2
		res = basePoint.Rotate(theta)
	} else {
		theta := math.Atan(rotatePoint.Y/rotatePoint.X) + math.Pi/2
		res = basePoint.Rotate(theta)
	}
