...
func main() {
		...
		if _, ok := m[c]; !ok {
			...
			if x > m[c].rightIndex {
				m[c] = struct {
					rightIndex int
					leftIndex  int
				}{rightIndex: x, leftIndex: m[c].leftIndex}
			}
			if x < m[c].leftIndex {
				m[c] = struct {
					rightIndex int
					leftIndex  int
				}{rightIndex: m[c].rightIndex, leftIndex: x}
			}
		}
	}
	m[maxColor+1] = struct {
	...
	for i := minColor; i <= maxColor+1; i++ {
		var tmpLeft, tmpRight struct {
			index int
			cost  int
		}
		indexes, ok := m[i]
		...
		commonLen := indexes.rightIndex - indexes.leftIndex
		leftStartCost := abs(indexes.rightIndex-left.index) + commonLen + left.cost
		rightStartCost := abs(indexes.rightIndex-right.index) + commonLen + right.cost
		tmpLeft.cost = min(leftStartCost, rightStartCost)
		tmpLeft.index = indexes.leftIndex
		leftStartCost = abs(indexes.leftIndex-left.index) + commonLen + left.cost
		rightStartCost = abs(indexes.leftIndex-right.index) + commonLen + right.cost
		tmpRight.cost = min(leftStartCost, rightStartCost)
		tmpRight.index = indexes.rightIndex
		left, right = tmpLeft, tmpRight
	}

	fmt.Println(left.cost)
