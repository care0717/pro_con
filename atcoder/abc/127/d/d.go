...
func main() {
	n, m := readInt(), readInt()
	as := readIntSlice(n)
	operators := make([]operator, m)
	for i := 0; i < m; i++ {
		operators[i] = operator{readInt(), readInt()}
	}
	pq := solve(n, m, as, operators)
	var sum int
...
func solve(n, m int, as []int, operators []operator) *PriorityQueue {
	items := make([]Item, n)
	for i, a := range as {
		items[i] = SimpleItem{C: a}
	}
	pq := NewPriorityQueue(items)
	sort.Slice(operators, func(i, j int) bool {
		return operators[i].c > operators[j].c
	})
	var i int
	var item Item
	for item, _ = pq.Pop(); i < m && operators[i].c > item.Cost(); item, _ = pq.Pop() {
		pq.Push(SimpleItem{
			C: operators[i].c,
		})
		for j := 0; j < (operators[i].b - 1); j++ {
			item, _ = pq.Pop()
			if operators[i].c > item.Cost() {
				pq.Push(SimpleItem{
					C: operators[i].c,
				})
			} else {
