...
func main() {
	...
	as := readIntSlice(n)
	sort.Ints(as)
	var minusNumCount, plusNumCount, minusToPlusIndex int
	var isPlus bool
	minusToPlusIndex = n
	for i, a := range as {
		if a < 0 {
	...
	if k <= minusNumCount*plusNumCount {
		...
		for left+1 < right {
			mid = (left + right) / 2
			count, sameCount := countMinusPair(as, mid, minusToPlusIndex)
			fmt.Println(left, right, count, sameCount)
			if k < count+sameCount {
				if count < k {
					left = mid
				}
				right = mid
	...
	k -= minusNumCount * plusNumCount
	var mid int64
	for left+1 < right {
		mid = (left + right) / 2
		counts := countXPairs(as, mid, minusToPlusIndex)
...
func countMinusPair(as []int, x int64, minusToPlusIndex int) (int, int) {
	n := len(as)
	i := minusToPlusIndex
	j := 0
	var count, sameCount int
	for i < n {
		for j < minusToPlusIndex {
			fmt.Println(int64(as[i]) * int64(as[j]))
			if x > int64(as[i])*int64(as[j]) {
				count += minusToPlusIndex - j
				break
			} else if x == int64(as[i])*int64(as[j]) {
				sameCount += minusToPlusIndex - j
				break
			} else {
				j++
			}
...
func countXPairs(as []int, x int64, minusToPlusIndex int) int {
	i := minusToPlusIndex - 1
	j := 0
	var count int
	for i > j {
		for i > j {
