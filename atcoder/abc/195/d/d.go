...
func main() {
	...
	sort.Slice(goods, func(i, j int) bool {
		return goods[i].V > goods[j].V
	})
	xs := readIntSlice(m)
	for i := 0; i < q; i++ {
		tmp := make([]int, len(xs)) // スライスtの骨組みを作成する
		copy(tmp, xs)
		l, r := readInt(), readInt()
		removedXs := append(tmp[:l-1], tmp[r:]...)
		sort.Ints(removedXs)

		fmt.Println(solve(removedXs, goods))
	}
}

func solve(xs []int, goods []struct {
	W int
	V int
}) int {
	var res int
