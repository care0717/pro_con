...
func (s Score) Incr(turn int, square uint8) Score {
	var diff int
	if square == '+' {
		diff = 1
	} else {
...
func main() {
	...
	scores := make([]Score, h*w)
	scores[start] = Score{0, 0}
	dq.Push(start)
	for !dq.Empty() {
		hw, _ := dq.Shift()
		height, width := hw/w, hw%w
		turn := (height + width + 1) % 2
		newScore := scores[hw].Incr(turn, board[height][width])
		if height > 0 {
			index := (height-1)*w + width
			if !marked[index] {
				dq.Push(index)
				marked[index] = true
				scores[index] = newScore
			} else if scores[index].Get(turn) < newScore.Get(turn) {
				scores[index] = newScore
			}
		}
		if width > 0 {
			index := height*w + width - 1
			if !marked[index] {
				dq.Push(index)
				marked[index] = true
				scores[index] = newScore
