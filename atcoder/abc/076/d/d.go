...
func main() {
	...
	tts := make([]int, sumTs+1)
	for i := 0; i < n; i++ {
		tts[ts[i]] = vs[i]
	}

	ttsReverse := make([]int, sumTs+1)
	for i := n - 1; i > 0; i-- {
		ttsReverse[ts[i-1]] = vs[i]
	}
	j := 0
	currentRangeMaxVs := make([]int, sumTs)
	for i := 0; i < sumTs; i++ {
		if tts[i] != 0 {
			j++
		}
		currentRangeMaxVs[i] = vs[j]
	}
	currentMaxVs := make([]int, sumTs+1)
	j = 0
	for i := 0; i <= sumTs; i++ {
		if tts[i] != 0 {
			j++
			if len(vs) == j {
				...
				currentMaxVs[i] = min(vs[j-1], vs[j])
			}
		} else {
			currentMaxVs[i] = vs[j]
		}
	...
	maxLeftVs := make([]int, sumTs+1)
	for i := 0; i <= sumTs; i++ {
		maxLeftVs[i] = 10000000000000
		if tts[i] != 0 {
			stacks[i] = tts[i]
		}
		for k, v := range stacks {
			maxLeftVs[i] = min(maxLeftVs[i], v-k+i)
		}
	}
	stacks = map[int]int{0: 0}
	maxRightVs := make([]int, sumTs+1)
	for i := sumTs; i >= 0; i-- {
		maxRightVs[i] = 10000000000000
		if ttsReverse[i] != 0 {
			stacks[sumTs-i] = ttsReverse[i]
		}
		for k, v := range stacks {
			maxRightVs[i] = min(maxRightVs[i], v-k+sumTs-i)
		}
	}
	var sum float64
	for i := 0; i < sumTs; i++ {
		currentSpeed := min(currentMaxVs[i], maxLeftVs[i], maxRightVs[i])
		nextSpeed := min(currentMaxVs[i+1], maxLeftVs[i+1], maxRightVs[i+1])
		if currentSpeed != nextSpeed {
			sum += float64(min(currentSpeed, nextSpeed)) + 0.5
		} else {
			sum += float64(currentSpeed)
			if currentSpeed < currentRangeMaxVs[i] {
				sum += 0.25
			}
		}
	}
...
func canRun(v, t, nextV, maxV int) (bool, float64) {
	if v-t > nextV || v+t < nextV || maxV < nextV {
		return false, 0
	}
	t0 := float64(t+nextV-v) / 2
	if float64(v)+t0 <= float64(maxV) {
		return true, (float64(v)+t0)*float64(t) - t0*t0/2 - (float64(t)-t0)*(float64(t)-t0)/2
	} else {
		// 台形になっているとき
		return true, float64(maxV*t) - float64((maxV-v)*(maxV-v))/2 - float64((maxV-nextV)*(maxV-nextV))/2
	}
}
