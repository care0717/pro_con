...
func main() {
	...
	for i := 0; i < size; i++ {
		...
		for _, diff := range []int{1, w, -1, -w} {
			newH := i/w + diff/w
			newW := i%w + diff%w
			if 0 <= newH && newH < h && 0 <= newW && newW < w && s[newH][newW] != '#' {
				nodes[i] = append(nodes[i], newH*w+newW)
			}
		}
	}
	var maxCost int
	for i := 0; i < size; i++ {
		if s[i/w][i%w] != '#' {
			cost := getLongestPathCost(h, w, i/w, i%w, nodes)
			if maxCost < cost {
				maxCost = cost
			}
		}
...
func getLongestPathCost(h, w, startH, startW int, nodes [][]int) int {
	...
	for pq.IsNotEmpty() {
		...
		if i.Priority > costs[index] {
			continue
		}
		for _, n := range nodes[index] {
			newCost := costs[index] + 1
			if costs[n] > newCost {
				costs[n] = newCost
				if maxCost < newCost {
					maxCost = newCost
