...
func main() {
	...
	halfLcm, err := calcHalfLcm(as)
	if err != nil || m < halfLcm {
		fmt.Println(0)
		return
	}
	fmt.Println((m-halfLcm)/lcm + 1)
}

func calcHalfLcm(as []int) (int, error) {
	lcm := calcLcm(as...)
	for _, a := range as {
		if lcm/a%2 == 0 {
			return 0, errors.New("half lcm does not exists")
		}
	}
	return lcm, nil
}
