...
func main() {
	n := readInt()
	exists := make(map[int]map[int]struct{})
	...
	for i := 0; i < n-1; i++ {
		...
		for j := i + 1; j < n; j++ {
			...
			if isExists(lowerRight, exists) && isExists(upperLeft, exists) {
				count++
			}
		}
	}
...
func isExists(p point, exists map[int]map[int]struct{}) bool {
	if ex, ok := exists[p.x]; ok {
		_, ok2 := ex[p.y]
		return ok2
	}
	return false
