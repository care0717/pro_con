...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		if edge, ok := edges[u]; ok {
			edges[u] = append(edge, v)
		} else {
			edges[u] = []int{v}
		}
		if edge, ok := edges[v]; ok {
			edges[v] = append(edge, u)
		} else {
			edges[v] = []int{u}
		}
	}
	...
	scores := solve(edges, start)
	if score, ok := scores["012345678"]; ok {
		fmt.Println(score)
	} else {
		fmt.Println(-1)
	}
...
func solve(edges map[int][]int, start []int) map[string]int {
	...
	for pq.IsNotEmpty() {
		...
		visited[i.Node()] = true
		costs[i.Node()] = i.Cost()
		puzzleItem := i.(PuzzleItem)
		to := puzzleItem.Find8()
		for _, from := range edges[to] {
			p := puzzleItem.puzzle
			newP := make([]int, len(p))
			copy(newP, p)
			tmp := newP[from]
...
func (p PuzzleItem) Priority() int {
	return p.C
}
func (p PuzzleItem) Cost() int {
	return p.C
}
func (p PuzzleItem) Node() string {
	var buffer bytes.Buffer
	for i := 0; i < len(p.puzzle); i++ {
		buffer.WriteString(strconv.Itoa(p.puzzle[i]))
		if i != len(p.puzzle)-1 {
			buffer.WriteString("")
		}
	}
...
func (p PuzzleItem) Find8() int {
	for i, num := range p.puzzle {
		if num == 8 {
			return i
		}
	}
