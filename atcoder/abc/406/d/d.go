...
func main() {
	...
	hs, ws := make(map[int]*garbage), make(map[int]*garbage)
	for i := 0; i < n; i++ {
		x, y := readInt(), readInt()
		g, ok := hs[x]
		if !ok {
	...
	var qs [][]int
	for i := 0; i < q; i++ {
		qs = append(qs, readIntSlice(2))
	}
	res := solve(h, w, hs, ws, qs)
	for _, r := range res {
		fmt.Println(r)
	}
}

func solve(h, w int, hs, ws map[int]*garbage, qs [][]int) []int {
	var res []int
	for _, q := range qs {
		if q[0] == 1 {
			g, ok := hs[q[1]]
