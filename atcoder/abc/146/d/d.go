...
func main() {
	...
	colorNum := max(vertexs...)

	fmt.Println(colorNum)
	edgeColors := solve(es)
	for _, e := range edgeColors {
		fmt.Println(e + 1)
	}

}

func solve(es [][]int) []int {
	n := len(es) + 1
	type edge struct {
		a           int
		b           int
	...
	adjacent := make([][]edge, n)

	for i, e := range es {
		adjacent[e[0]] = append(adjacent[e[0]], edge{
			a:     e[0],
			b:     e[1],
			index: i,
		})
		adjacent[e[1]] = append(adjacent[e[1]], edge{
			a:     e[1],
			b:     e[0],
			index: i,
		})
	...
	edgeColors := make([]int, n-1)
	visited := make([]bool, n)
	for len(queue) > 0 {
		q, rest := queue[0], queue[1:]
		var color int
		for _, e := range adjacent[q] {
			if visited[e.b] {
				continue
			}
			if colors[q] == color {
			...
			edgeColors[e.index] = color
			colors[e.b] = color
			color++
			rest = append(rest, e.b)
			visited[q] = true
		}
		queue = rest
	}
