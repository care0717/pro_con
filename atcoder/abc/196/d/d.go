...
func main() {
	h, w, a, _ := readInt(), readInt(), readInt(), readInt()
	space := make([][]bool, h)
	for i := 0; i < h; i++ {
		space[i] = make([]bool, w)
	}
	...
	fmt.Println(solve(space, h, w, a) / fact(a, 0))
}

func solve(space [][]bool, h, w, a int) int {
	settings := calcSettings(space, h, w)
	if a == 1 {
		return len(settings)
	}
	var sum int
	for _, s := range settings {
		newSpace := make([][]bool, h)
		for i := 0; i < h; i++ {
			newSpace[i] = make([]bool, w)
			copy(newSpace[i], space[i])
		}
		newSpace[s.hIndex][s.wIndex] = true
		if s.isVertical {
			newSpace[s.hIndex+1][s.wIndex] = true
		} else {
			newSpace[s.hIndex][s.wIndex+1] = true
		}
		sum += solve(newSpace, h, w, a-1)
	}
	return sum
}

func calcSettings(space [][]bool, h, w int) []setting {
