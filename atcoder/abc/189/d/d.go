...
func main() {
	...
	for _, s := range ss {
		trueCount, falseCount = getTrueFalseCount(trueCount, falseCount, s)
	}
	fmt.Println(trueCount)
}

func getTrueFalseCount(trueCount, falseCount int64, s string) (int64, int64) {
	if s == "OR" {
		trueCount = trueCount*2 + falseCount
	} else {
		falseCount = trueCount + falseCount*2
	}
