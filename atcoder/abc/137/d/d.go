...
func (pq *PriorityQueue) Concat(other *PriorityQueue) {
	if pq.Len() > other.Len() {
		for other.IsNotEmpty() {
			i, _ := other.Pop()
			pq.Push(i)
		}
	} else {
...
func main() {
	...
	rewardQueues := make([]*PriorityQueue, maxDay+1)
	for i := 0; i < maxDay+1; i++ {
		rewardQueues[i] = NewPriorityQueue(nil)
	}

	for _, r := range rewards {
		rewardQueues[r.day].Push(&Item{
			Reward:   r.r,
			Priority: -r.r,
		})
	}
	var maxReward int
	for i := 0; i < m; i++ {
		rewardQueues[i+1].Concat(rewardQueues[i])
		item, err := rewardQueues[i+1].Pop()
		if err != nil {
			continue
		}
