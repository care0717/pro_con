...
func main() {
	...
	for i, c := range s {
		if current != c {
			if current == 'L' && c == 'R' {
				rls = append(rls, s[rIndex:i])
				rIndex = i
			}
			current = c
		}
	}
	rls = append(rls, s[rIndex:n])
	...
	var index int
	for _, rl := range rls {
		rCount, lCount := solve(rl)
		i := strings.Index(rl, "L")
		result[index+i-1] = fmt.Sprintf("%d", rCount)
		result[index+i] = fmt.Sprintf("%d", lCount)
		index += len(rl)
	}
...
func solve(rl string) (int, int) {
	r := strings.Count(rl, "R")
	l := strings.Count(rl, "L")
