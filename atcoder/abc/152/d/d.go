...
func main() {
	...
	for h := 1; h < 10; h++ {
		for t := 1; t < 10; t++ {
			m[h][t] = countHeadTailNumUnderN(n, h, t)
		}
	}
	var count int
	for h := 1; h < 10; h++ {
...
func countHeadTailNumUnderN(n, head, tail int) int {
	digit := 1
	for {
		if n >= head*digit*10+tail {
			digit *= 10
		} else {
			break
		}
	...
	minHeadTailNum := head*digit + tail
	if n/digit == head {
		for (n-minHeadTailNum)%10 != 0 {
			n--
		}
		count += (n-minHeadTailNum)/10 + 1
	} else {
		count += digit / 10
	}
