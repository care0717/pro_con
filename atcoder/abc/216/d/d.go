...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		first := as[0]
		counts[first] = append(counts[first], i)
		ass[i] = as
	}
	var queue [][]int
	...
	for len(queue) > 0 {
		...
		queue = queue[1:]
		popCount++
		p1, p2 := first[0], first[1]
		ass[p1] = ass[p1][1:]
		ass[p2] = ass[p2][1:]
		if len(ass[p1]) > 0 {
			newValue := ass[p1][0]
			counts[newValue] = append(counts[newValue], p1)
			if len(counts[newValue]) > 1 {
				queue = append(queue, counts[newValue])
			}
		}
		if len(ass[p2]) > 0 {
			newValue := ass[p2][0]
			counts[newValue] = append(counts[newValue], p2)
			if len(counts[newValue]) > 1 {
				queue = append(queue, counts[newValue])
			}
		}
